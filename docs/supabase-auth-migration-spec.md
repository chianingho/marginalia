...
> ## ⚠️ 本節已作廢(2026-07-23 裁示)
>
> **本節所述的一次性遷移函式取消,不實作。**
>
> **原因**:2026-07-23 遷移前盤點確認,本機已無真實資料可遷移。
> iPhone Safari 上 7/6 建立的書與筆記已全部消失;桌機兩個網域的殘留資料
> 全部缺少 `status` 與 `added_at` 欄位,判定為範例書或 7/9 加欄位之前的舊測試資料,
> 非真實閱讀紀錄。
>
> 資料量歸零後,一批高風險、不可逆、且只會執行一次的程式碼失去全部價值。
>
> **連帶取消**:
> - `marginalia_migrated` 旗標
> - 跨裝置圖片路徑重算的隱患處理
> - 遷移前的 localStorage 救援備份
>
> **原文以下全部保留,供日後追溯決策脈絡,不刪除。**
